...
| 模块 | 当前正式选择 | 说明 |
| --- | --- | --- |
| 视觉识别 | VISS v28 与 Qwen 优先识别 | Qwen 全局识别候选，YOLO11s-seg-best 精分割，Qwen/视觉语言模型给四分类标签 |
| 相机数据 | 当前主场景 `head_rgbd` | 使用 Task319 场景自身相机内参、深度图和相机位姿，不使用 VISS 原始偏置相机参数 |
| 3D 定位 | RGB-D 目标区域内几何中心和顶部抓取点 | 抓取点来自当前站位重新拍摄的 RGB-D，不使用上帝视角作为主线定位 |
...
| 演示闭环 | 夹爪近距辅助携带路径 | 用于跑通完整视觉、导航、分类、投放链路，不作为物理夹爪验收 |
| RL 后续 | skrl SAC/PPO 局部接近-抓取策略 | 正在训练接管最后下降、闭合和抬升阶段 |
...
该记录完成了 8 个物体的视觉识别、分类、导航到桶侧和桶口上方释放；第 9 次返回观察位时，机器人未能在限定时间内到达预定观察点，因此全局任务在返回阶段结束。此前已完成物体没有投放失败。该记录使用了夹爪近距辅助携带路径，必须作为展示闭环而非真实夹爪最终成功来解读。

| 轮次 | 目标编号 | 识别名称 | 分类 | 目标桶 | 轮次成功 | 桶口释放 | 物体瞬移 |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
1. Qwen/视觉语言模型先看全局 RGB 图，返回高可信度垃圾候选、语义名称和四分类类别。
2. YOLO11s-seg-best 对候选局部区域做精确分割。
3. Qwen 对局部图像做验证和分类确认。
4. Task319 将二维目标区域绑定到当前 RGB-D 深度图，计算该物体在世界坐标下的几何信息。
5. 分类标签以视觉语言模型输出为准，不再使用硬编码类别作为正式分类结果。
...
- 视觉语言模型如果只看裁剪得过干净的局部物体图像，容易因为缺少上下文而误判。
- GLM/旧链路和当前场景相机参数不统一，导致分类、定位和抓取前后不一致。
- 早期有候选数量限制，后续多物体场景中容易漏检。

### 当前解决方案

- 主线统一成 VISS v28 与 Qwen 优先识别链路，彻底弃用 GLM 和旧 YOLO 作为默认链路。
- 使用当前 Task319 场景的头部 RGB-D 和相机外参，不使用 VISS 原始偏置相机参数。
- 每个任务轮次先在观察位选目标，再导航到站位重新拍照，抓取定位使用站位处的最新 RGB-D。
- 分类标签绑定到当前选中的实例，不跨物体复用。
- 默认不限制识别个数，避免多物体桌面漏检。
- 保存站位处识别结果、叠加可视化图、目标分割区域和单轮结构化记录，便于追溯“标红目标”和分类标签来源。
...
- 通过 VISS v28 的目标分割区域取对应 RGB-D 像素。
- 使用当前 intrinsics 和 `camera_to_world` 将深度点投到世界坐标。
- 计算物体 3D 几何中心、点云范围和最高点 `Z_max`。
- 顶部抓取路径使用物体水平中心和顶部高度附近的浅层目标，而不是直接使用物体几何中心高度。
- 预备抓取点位于物体上方，最后阶段应尽量垂直下降，避免斜向扫到物体。
...
- 抓取高度转向顶部抓取，减少从几何中心下插导致的碰撞。
- 默认主线避免严格轴向对齐，优先保证位置接近和路径不撞物体。
- 物理夹爪不稳定时，演示链路使用近距辅助机制：末端进入夹爪附近范围后，将物体绑定到右夹爪末端并携带到垃圾桶。
...
1. GraspNet/AnyGrasp：
   - 添加过点云合理性检查、点云保存、分割区域距离放宽、保留更多抓取候选、关闭逆运动学预筛选等诊断。
   - 实际问题是 GraspNet 原始候选置信度经常为零，或候选进入执行后仍在运行时逆运动学/安全起始阶段失败。
...
3. cuRobo：
   - 用于右臂物体上方预备抓取姿态规划和部分空间下降尝试。
   - 遇到的问题是冗余姿态和位置-only IK 分支选择可能让手臂出现视觉上不合理的扭转。
   - 后续策略是减少不必要姿态约束，更多用于避障和到物体上方预备点的全局规划，最后下降交给局部闭环或强化学习策略。
...
真实物理夹爪仍是当前最大未闭环问题。为了推进系统级展示，主线保留了近距辅助携带演示路径，但报告和 README 必须明确区分：

- 真实物理夹爪验收：需要物体被手指闭合夹住、抬升、随末端运动。
- 当前完整演示闭环：使用真实 VISS v28 识别、真实 Nav2 导航、真实桶口末端释放，但抓取阶段可用近距辅助携带保证视频完整。

后续应将 SAC 训练出的局部接近-抓取策略接入真实物理夹爪路径，减少手写开环下降和接触参数调试成本。
...
- Nav2 偶发返回观察位未在限定时间内到达的问题，例如最新完整演示在返回观察位时结束。当前需要增加更稳健的重试、软成功和定位容差处理。
...
- 当前演示链路已经改成：机器人先导航到目标桶侧，右夹爪末端再移动到对应桶口上方，然后打开夹爪或释放辅助携带。
...
训练目标非常明确：不训练完整“看图到扔垃圾”，只训练最后到达物体上方预备点后 10-15 cm 的局部抓取控制。
...
2. 用现有规划器把右夹爪移动到目标上方预备点。
3. 将当前 RGB-D 目标转换成局部策略的观测。
4. 最后下降、闭合和抬升由 SAC 策略每帧输出小动作。
...
| 问题 | 根因 | 已采取方案 | 当前状态 |
| --- | --- | --- | --- |
| 视觉模块选中物体不一致 | YOLO、视觉语言模型、抓取候选没有绑定同一实例 | 统一到 VISS v28 与 Qwen 优先识别链路，单轮内绑定实例 | 基本解决 |
| GLM/旧 YOLO 效果差 | 模型链路和当前场景相机/任务不匹配 | 弃用 GLM 和旧 YOLO 默认主线 | 已解决 |
| GraspNet 零候选 | 抓取候选置信度低、点云薄、内部碰撞过滤、目标区域或坐标误差 | 加点云保存、阈值放宽、保留更多候选、关闭预筛选 | 不作为默认 |
...
| 夹爪碰飞物体 | 下降非直线、闭合过早、目标高度不合理 | 顶部抓取、物体上方预备点、慢下降、慢闭合 | 仍是核心问题 |
| 投放瞬移不真实 | 早期直接写物体 pose 到桶里 | 改为末端先到桶口上方再释放 | 已改善 |
| 底盘 yaw 抖 | 轮子物理接触和映射不稳定 | `urdf_diagonal + kinematic_stable` | 明显改善 |
| 返回观察位未在限定时间内到达 | 终点残差、局部控制器退出状态、长循环累积 | 软成功、重试、返回观察位策略 | 仍偶发 |
...
1. 接入 SAC 局部接近-抓取策略：
   - 先在小方块和标准垃圾物体上评估成功率。
   - 再替换主线严格物理抓取的最后下降、闭合和抬升。
   - 保留近距辅助携带只做展示备用路径，不用于物理验收。

2. 强化抓取前闭环重定位：
   - 导航到站位后必须重新拍 RGB-D。
   - 若末端到达物体上方预备点后目标发生移动，再用局部相机/RGB-D 刷新目标。
   - 可以考虑手腕相机作为最后 10 cm 的视觉伺服输入。

3. 减少 IK 病态姿态：
   - cuRobo 负责到物体上方预备点的避障规划。
   - 最后下降使用局部闭环或密集空间路点。
   - 姿态只保留必要约束，不强求无意义轴向完全对齐。

4. 提升 Nav2 稳定性：
   - 对返回观察位未在限定时间内到达的问题增加局部重试和容差判断。
   - 在关键拍照位做更稳定的面向桌面/垃圾桶 yaw 对齐。
   - 继续保留 `kinematic_stable`，避免回到纯接触轮地导致不可动。

5. 完善文档和验收：
   - 每次视频保存后自动退出。
   - 每个完整实验记录命令、版本、分类表、失败原因。
