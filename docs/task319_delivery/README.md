...
-> VISS v28 Qwen 优先识别 + YOLO11 局部区域分割
-> RGB-D 目标三维定位和顶部抓取姿态
-> Nav2 动态桌边站位
-> 抓取或演示辅助抓取
-> Nav2 到对应垃圾桶
...
严格物理抓取仍在调试。完整系统演示视频使用显式的夹爪近距辅助携带路径跑通全流程；严格物理抓取验证命令会在失败时停止并记录原因。
...
| 参考资料归档 | `docs/task319_delivery/references/` |
...
除仓库根 `README.md` 外，Task319 文档交付材料统一放在本目录。`references/` 中保留开发过程报告、算法说明、命令手册、导航实现记录、抓取 baseline、参考论文 PDF 和历史计划文档，便于答辩追溯。
...
| 模型 | 获取位置 | 本地放置路径 | 用途 |
| --- | --- | --- | --- |
| YOLO11 垃圾分割主线权重 | 项目交付模型附件/团队模型盘 | `viss/models/yolo11s-seg-best.pt` | VISS v28 主线局部区域分割 |
...
当前主线 GUI 命令，默认严格物理抓取，失败会记录并停止对应物体处理轮次：

```bash
python task_319_garbage_sort/task319_grasp_sort_sm.py --mind_sort_demo
```

完整系统演示视频命令，显式启用夹爪近距辅助携带路径，用于展示识别、导航、分类、投放闭环：
...
GitHub 提交版视频由原始 `grasp8.mp4` 压缩生成，保留完整时长，分辨率 854x480，帧率 10 fps。该实验的 `video_manifest.json` 仍记录原始输出名 `external_grasp_demo.mp4`，但当前目录中实际保留的视频文件名是 `grasp8.mp4`。该实验完成 8 个物体的识别、分类和桶口释放；第 9 次返回观察位时，机器人未能在限定时间内回到预定观察点，此前已完成轮次均为成功。

| 轮次 | 物体 | 分类 | 轮次结果 | 严格物理抓取 | 辅助抓取 | 投放 |
| --- | --- | --- | --- | --- | --- | --- |
...
结果：`trash_potted_meat_can_0` 单物体严格物理抓取失败，原因是机械臂末端在最后下降阶段未到达预定抓取位置，未启用辅助携带路径。
...
该记录是逻辑 pick-and-place 动画，用于验证场景对象、类别映射和投放动作编排。10 个任务的逻辑指标均为 100%。

课程报告 `docs/task319_delivery/COURSE_REPORT.md` 采用三组代表性实验记录汇总指标：10 任务逻辑动画、8 个完整主线物体处理轮次、1 个严格物理单轮验证。项目开发期间还进行了约为代表性记录 10-20 倍的历史调试测试，覆盖视觉、站位、Nav2、抓取和投放参数迭代，用于说明工程覆盖度和稳定性优化过程。
...
5. 返回观察位阶段未在限定时间内到达：
   这是已知问题，通常发生在长循环返回观察位或末端对位残差阶段。单个已完成轮次的投放记录仍应以对应 `mind_sort_cycle.json` 为准。

6. 物理抓取失败：
   当前严格物理夹爪仍是主要未闭环项。调试入口包括 `topdown_cube_grasp_test.py`、`gripper_physics_test.py`、`task319_local_grasp_rl/`。

7. 不应把辅助演示当作物理夹爪验收：
   启用 `--mind_sort_suction_assist` 或 `--mind_sort_gripper_proximity_assist` 的实验只证明系统级闭环，不证明真实夹爪已经稳定抓住所有垃圾。
