...
# Add scripts directory to path to allow importing from neighboring scripts
sys.path.append(str(Path.home() / "trashbot_ws" / "scripts" / "perception"))
from yolo11_qwen_perception_offline import classify_crop_with_qwen
...
PLAN_JSON = Path("/mnt/d/isaac_projects/v28_visual_task_plan.json")
ACTION_RESULT_JSON = Path("/mnt/d/isaac_projects/v28_head_camera_action_result.json")

RUN_LOG_JSON = LOG_DIR / "v28_two_stage_run_log.json"
RUN_LOG_CSV = LOG_DIR / "v28_two_stage_run_table.csv"
...
def run_yolo_fast(image_path: Path, conf: float, args, save_overlay=False, backend=None):
    ...
    if backend in ["yolo11_qwen", "qwen_first", "yolo_first"]:
        ...
        if worker_ready and worker_ready.get("status") == "ready":
            ...
            num_qwen_cands = result_data.get("num_qwen_candidates", 0)
            num_dets = result_data.get("num_detections", 0)
            print(f"[QWEN WORKER IPC] Qwen coarse candidates count: {num_qwen_cands}, final detections: {num_dets}")
            return result_data
        ...
        num_qwen_cands = result_data.get("num_qwen_candidates", 0)
        num_dets = result_data.get("num_detections", 0)
        print(f"[QWEN SUBPROCESS] Qwen coarse candidates count: {num_qwen_cands}, final detections: {num_dets}")
        return result_data
...
# QWEN VERIFY category override rules (raw Qwen output → correct bin)
# Applied to override coarse YOLO classification after pick
QWEN_CATEGORY_TO_BIN = {
...
def make_plan(args, blocked_raw, selected_view, cycle_id):
    ...
    if plan.get("selected_task") is not None:
        ...
        plan["mode"] = "V2_8_HEAD_CAMERA_TWO_STAGE_PLAN"
...
PRIM_TO_EXPECTED_BIN = {
    "trash_battery": "bin_hazardous_red",
    "trash_medicine_box": "bin_hazardous_red",
    "trash_can": "bin_recyclable_blue",
    "trash_dirty_bottle": "bin_recyclable_blue",
    "trash_plastic_bottle": "bin_recyclable_blue",
    "trash_paper_box": "bin_recyclable_blue",
    "trash_tissue": "bin_recyclable_blue",
    "trash_broken_cup": "bin_other_gray",
    "trash_apple_core": "bin_kitchen_green",
    "trash_banana_peel": "bin_kitchen_green",
}

def check_semantic_correct(attached_object_prim, pred_bin):
    if not attached_object_prim or not pred_bin:
        return False
    prim_name = attached_object_prim.split("/")[-1].lower()
    for key, expected_bin in PRIM_TO_EXPECTED_BIN.items():
        if key in prim_name:
            return pred_bin == expected_bin
    return False
...
def compute_metrics(records):
    if not records:
        return {
            ...
            "average_pick_sec": 0.0,
            "average_place_sec": 0.0,
            "average_verify_sec": 0.0,
            "average_verify_wait_after_pick_sec": 0.0,
        }

    total = len(records)
    
    # pick_success_rate
    pick_ok = sum(1 for r in records if r.get("pick_only_sec", 0.0) > 0.0 and r.get("action_result", {}).get("pick_completed") is True)
    
    # place_success_rate
    place_ok = sum(1 for r in records if r.get("place_held_sec") is not None and r.get("place_held_sec", 0.0) > 0.0 and r.get("action_result", {}).get("place_completed") is True)
    
    # action_success_rate (action_result.status == "success")
    action_ok = sum(1 for r in records if r.get("action_result", {}).get("status") == "success")
    
    # verification_success_rate (verification.verified is True)
    verify_ok = sum(1 for r in records if r.get("verification", {}).get("verified") is True)

    # semantic_correct_rate
    semantic_correct_count = 0
    semantic_audited_count = 0
    for r in records:
        attached_prim = r.get("action_result", {}).get("attached_object_prim") or r.get("action_result", {}).get("held_object_prim")
        if attached_prim:
            pred_bin = r.get("target_bin_after_verify")
            if check_semantic_correct(attached_prim, pred_bin):
                semantic_correct_count += 1
            semantic_audited_count += 1

    semantic_correct_rate = semantic_correct_count / semantic_audited_count if semantic_audited_count > 0 else 0.0
    ...
    pick_secs = [float(r["pick_only_sec"]) for r in records if r.get("pick_only_sec") is not None and r.get("pick_only_sec") > 0.0]
    place_secs = [float(r["place_held_sec"]) for r in records if r.get("place_held_sec") is not None and r.get("place_held_sec") > 0.0]
    verify_secs = [float(r["qwen_verify_sec"]) for r in records if r.get("qwen_verify_sec") is not None and r.get("qwen_verify_sec") > 0.0]
    verify_wait_secs = [float(r["verify_wait_after_pick_sec"]) for r in records if r.get("verify_wait_after_pick_sec") is not None]

    return {
        "num_cycles": total,
        "pick_success_rate": round(pick_ok / total, 4),
        "place_success_rate": round(place_ok / total, 4),
        "action_success_rate": round(action_ok / total, 4),
        "verification_success_rate": round(verify_ok / total, 4),
        "semantic_correct_rate": round(semantic_correct_rate, 4),
        "average_attach_distance_xy": round(sum(attach) / len(attach), 4) if attach else 0.0,
        "average_pick_sec": round(sum(pick_secs) / len(pick_secs), 4) if pick_secs else 0.0,
        "average_place_sec": round(sum(place_secs) / len(place_secs), 4) if place_secs else 0.0,
        "average_verify_sec": round(sum(verify_secs) / len(verify_secs), 4) if verify_secs else 0.0,
        "average_verify_wait_after_pick_sec": round(sum(verify_wait_secs) / len(verify_wait_secs), 4) if verify_wait_secs else 0.0,
    }

def save_logs(records, extra=None):
    summary = {
        "mode": "V2_8_HEAD_CAMERA_TWO_STAGE_LOOP",
    ...
    with open(RUN_LOG_CSV, "w", encoding="utf-8-sig", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            ...
            "qwen_verify_started_before_pick",
            "qwen_verify_overlap_with_pick",
            "qwen_verify_sec",
            "pick_only_sec",
            "place_held_sec",
            "verify_wait_after_pick_sec",
            "verify_used_for_target_bin",
            "verify_fallback_used",
            "verify_fallback_reason",
            "target_bin_before_verify",
            "target_bin_after_verify",
        ], extrasaction='ignore')
        writer.writeheader()
        ...
        for r in records:
            ...
            writer.writerow({
                ...
                "qwen_verify_started_before_pick": r.get("qwen_verify_started_before_pick"),
                "qwen_verify_overlap_with_pick": r.get("qwen_verify_overlap_with_pick"),
                "qwen_verify_sec": r.get("qwen_verify_sec"),
                "pick_only_sec": r.get("pick_only_sec"),
                "place_held_sec": r.get("place_held_sec"),
                "verify_wait_after_pick_sec": r.get("verify_wait_after_pick_sec"),
                "verify_used_for_target_bin": r.get("verify_used_for_target_bin"),
                "verify_fallback_used": r.get("verify_fallback_used"),
                "verify_fallback_reason": r.get("verify_fallback_reason"),
                "target_bin_before_verify": r.get("target_bin_before_verify"),
                "target_bin_after_verify": r.get("target_bin_after_verify"),
            })
...
def main():
    ...
    parser.add_argument("--qwen-first-verify-mode", default="none",
    ...
    try:
        ...
        print("=" * 80)
        print("[START] V2.8 two-stage pick_only + place_held loop")
        print("[INFO] Isaac must be running:")
        print("  1) v26_active_view_search_isaac_server.py")
        print("  2) v28_head_camera_two_stage_action_server.py")
        print("=" * 80)
        ...
        for cycle_id in range(1, args.max_cycles + 1):
            ...
            # ====================================================================
            # V2.8 TWO-STAGE: pick_only + concurrent qwen verify + place_held
            # ====================================================================
            target_bin_before_verify = selected_task.get("target_bin", "bin_other_gray")
            raw_class = selected_task.get("raw_class_name", "trash_object")
            crop_path_str = selected_task.get("crop_path") or selected_task.get("crop_image")

            # ---- Stage 1: submit background Qwen verify while pick happens ----
            qwen_future = None
            qwen_verify_started_before_pick = False
            qwen_verify_t0 = None
            ...
            if crop_path_str:
                ...
                if crop_path.exists():
                    print(f"[V28 VERIFY SUBMIT] submitting background Qwen verify for crop={crop_path.name}")
                    qwen_verify_t0 = time.time()
                    qwen_future = executor.submit(classify_crop_with_qwen, crop_path, raw_class)
                    qwen_verify_started_before_pick = True
                else:
                    print(f"[V28 VERIFY SKIP] crop_path not found: {crop_path_str}")
            else:
                print("[V28 VERIFY SKIP] no crop_path in selected_task")

            # ---- Stage 1: write pick_only plan and wait ----
            plan["action_phase"] = "pick_only"
            save_json_atomic(PLAN_JSON, plan)
            print(f"[V28 PICK_ONLY] plan_id={plan_id} -> waiting for pick result")

            t_pick_start = time.time()
            pick_result = wait_action_result(plan_id, args.action_timeout_sec)
            pick_only_sec = round(time.time() - t_pick_start, 3)

            print(f"[V28 PICK_ONLY RESULT] status={pick_result.get('status')} elapsed={pick_only_sec}s")

            pick_ok = pick_result.get("status") == "pick_success"
            held_object_prim = pick_result.get("held_object_prim") or pick_result.get("attached_object_prim")

            if not pick_ok:
                # Pick failed – no place, log and continue
                executor.shutdown(wait=False)
                failed_raw = pick_result.get("raw_class_name")
                failed_status = pick_result.get("status")
                if failed_raw and failed_status == "failed_no_matching_object":
                    ...
                    print(f"[V28 PICK FAILED] raw_class={failed_raw}, status={failed_status}")

                verification = {
                    "verified": False,
                    "reason": "pick_only_failed",
                    "action_status": pick_result.get("status"),
                }
                record = {
                    "cycle_id": cycle_id,
                    "plan_id": plan_id,
                    ...
                    "after_yolo_inference_sec": None,
                    "action_result": pick_result,
                    "verification": verification,
                    "cycle_sec": round(time.time() - cycle_t0, 4),
                    "pick_only_sec": pick_only_sec,
                    "place_held_sec": None,
                    "qwen_verify_started_before_pick": qwen_verify_started_before_pick,
                    "qwen_verify_overlap_with_pick": qwen_verify_started_before_pick,
                    "qwen_verify_sec": None,
                    "verify_wait_after_pick_sec": None,
                    "verify_used_for_target_bin": False,
                    "verify_fallback_used": True,
                    "verify_fallback_reason": "pick_failed",
                    "target_bin_before_verify": target_bin_before_verify,
                    "target_bin_after_verify": target_bin_before_verify,
                }
                record.update(perception_meta)
                records.append(record)
                save_logs(records)
                if args.stop_on_failure:
                    print("[STOP] failure detected after pick_only failed.")
                    break
                continue

            # ---- Stage 2: collect Qwen verify result (up to 5s after pick) ----
            VERIFY_TOTAL_BUDGET_SEC = 5.0
            verify_deadline = (qwen_verify_t0 or time.time()) + VERIFY_TOTAL_BUDGET_SEC
            remaining_sec = max(0.0, verify_deadline - time.time())

            target_bin_after_verify = target_bin_before_verify
            qwen_verify_sec = None
            verify_used = False
            verify_fallback_used = False
            verify_fallback_reason = "none"

            if qwen_future is not None:
                t_wait = time.time()
                try:
                    qwen_res = qwen_future.result(timeout=remaining_sec)
                    qwen_verify_sec = round(time.time() - qwen_verify_t0, 3) if qwen_verify_t0 else None
                    verify_wait_after_pick_sec = round(time.time() - t_wait, 3)

                    verify_category = qwen_res.get("garbage_category", "")
                    verify_bin = QWEN_CATEGORY_TO_BIN.get(verify_category)

                    if verify_bin and verify_category not in ("", "unknown"):
                        print(f"[V28 VERIFY OVERRIDE] coarse_bin={target_bin_before_verify} -> verify_bin={verify_bin} (category={verify_category})")
                        target_bin_after_verify = verify_bin
                        verify_used = True
                    else:
                        print(f"[V28 VERIFY FALLBACK] verify_category={verify_category} not usable, keeping coarse bin={target_bin_before_verify}")
                        verify_fallback_used = True
                        verify_fallback_reason = f"unknown_or_empty_category: {verify_category}"

                except Exception as e:
                    qwen_verify_sec = round(time.time() - qwen_verify_t0, 3) if qwen_verify_t0 else None
                    verify_wait_after_pick_sec = round(time.time() - t_wait, 3)
                    verify_fallback_used = True
                    verify_fallback_reason = f"qwen_verify_exception: {repr(e)}"
                    print(f"[V28 VERIFY TIMEOUT/ERROR] keeping coarse bin={target_bin_before_verify}, reason={repr(e)}")
            else:
                verify_wait_after_pick_sec = 0.0
                verify_fallback_used = True
                verify_fallback_reason = "no_crop_path"

            executor.shutdown(wait=False)

            # ---- Stage 2: update selected_task with verified bin/category and write place_held plan ----
            place_plan_id = f"{plan_id}_place"
            updated_task = dict(selected_task)
            updated_task["target_bin"] = target_bin_after_verify
            updated_task["held_object_prim"] = held_object_prim

            # Map target_bin back to garbage_category
            bin_to_category = {
                "bin_recyclable_blue": "recyclable",
                "bin_kitchen_green": "kitchen",
                "bin_hazardous_red": "hazardous",
                "bin_other_gray": "other",
            }
            updated_task["garbage_category"] = bin_to_category.get(target_bin_after_verify, selected_task.get("garbage_category", "other"))

            place_plan = dict(plan)
            place_plan["plan_id"] = place_plan_id
            place_plan["action_phase"] = "place_held"
            place_plan["selected_task"] = updated_task
            place_plan["held_object_prim"] = held_object_prim
            save_json_atomic(PLAN_JSON, place_plan)
            print(f"[V28 PLACE_HELD] plan_id={place_plan_id}, bin={target_bin_after_verify}, held_prim={held_object_prim}")

            t_place_start = time.time()
            place_result = wait_action_result(place_plan_id, args.action_timeout_sec)
            place_held_sec = round(time.time() - t_place_start, 3)

            print(f"[V28 PLACE_HELD RESULT] status={place_result.get('status')} elapsed={place_held_sec}s")

            # ---- Build merged action_result and verification ----
            action_result = dict(place_result)
            action_result.update({
                "pick_completed": pick_result.get("pick_completed", True),
                "place_completed": place_result.get("place_completed", place_result.get("status") == "success"),
                "attached_object_prim": held_object_prim,
                "held_object_prim": held_object_prim,
                "attach_distance_xy": pick_result.get("attach_distance_xy"),
                "raw_class_name": pick_result.get("raw_class_name", raw_class),
                "garbage_category": updated_task.get("garbage_category"),
                "target_bin": target_bin_after_verify,
            })
            ...
            if action_ok:
                verification = {
                    ...
                    "reason": "action_result_hidden_after_drop",
                    "qwen_first_mode": True,
                    "action_hidden_primary_vote": True,
                    "aux_vote_score": 0,
                    "vote_total_decreased": False,
                    "vote_category_decreased": False,
                    "vote_raw_class_decreased": False,
                    "selected_raw_class": raw_class,
                    "selected_category": updated_task.get("garbage_category"),
                }
            else:
                verification = {
                    "verified": False,
                    "reason": "place_held_failed",
                    "action_status": place_result.get("status"),
                    "selected_raw_class": raw_class,
                    "selected_category": updated_task.get("garbage_category"),
                }
            ...
            record = {
                ...
                "cycle_sec": round(time.time() - cycle_t0, 4),
                "pick_only_sec": pick_only_sec,
                "place_held_sec": place_held_sec,
                "qwen_verify_started_before_pick": qwen_verify_started_before_pick,
                "qwen_verify_overlap_with_pick": qwen_verify_started_before_pick,
                "qwen_verify_sec": qwen_verify_sec,
                "verify_wait_after_pick_sec": verify_wait_after_pick_sec,
                "verify_used_for_target_bin": verify_used,
                "verify_fallback_used": verify_fallback_used,
                "verify_fallback_reason": verify_fallback_reason,
                "target_bin_before_verify": target_bin_before_verify,
                "target_bin_after_verify": target_bin_after_verify,
            }
            record.update(perception_meta)
